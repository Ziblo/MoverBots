...
bool MOdrive::calibrate(ODriveArduino* o, int axis){
    ...
    if(!o->run_state(axis, AXIS_STATE_MOTOR_CALIBRATION, true, 5.0f)) return false;
...
void MOdrive::clear_errors(){
    Serial << "Clearing Errors" << '\n';
    ODRIVE_SERIAL_1 << "w axis" << axis_1 << ".error 0" << '\n';
    ODRIVE_SERIAL_2 << "w axis" << axis_2 << ".error 0" << '\n';
}